{ config, lib, ... }:
let
  cfg = config.mine.home.neomutt;
in
{
  options.mine.home.neomutt = {
    enable = lib.mkEnableOption "Enable neomutt config";
  };
  config = lib.mkIf cfg.enable {
    accounts.email.accounts.personal = {
      address = lib.strings.trim (builtins.readFile /home/${config.mine.home.user.name}/.ironman/personal-email);
      primary = true;
    };
  };
}
