  ...
  config = mkIf cfg.enable {
    mine = {
      servers.postgresql = {
        ...
        '';
        # ${config.mine.networking.basic.network}/${config.mine.networking.basic.prefix} all sonarqube peer
        enable = true;
        script = [
          ''
    ...
    networking.firewall = mkIf config.mine.networking.basic.firewall {allowedTCPPorts = [9000];};
